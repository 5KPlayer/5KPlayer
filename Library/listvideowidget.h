#ifndef LISTVIDEOWIDGET_H
#define LISTVIDEOWIDGET_H
...
class QScrollArea;
class ListsVideo;
class ListVideoWidget : public TransEventWidget, public UpdateInterface
{
    Q_OBJECT
public:
    explicit ListVideoWidget(QWidget *parent = 0);
    ~ListVideoWidget();
    void updateLanguage();
...
private:
    QList<ListsVideo *> wlist;//子控件
    QString _showItemName;  //当前界面显示的item

    ListsVideo *_chooseWidget;//鼠标点中的界面

    QWidget*    _pListWidget;   // listWidget主界面
